...
int main()
{
    ...
    // see: https://github.com/unicode-org/icu/blob/master/icu4c/source/samples/ucnv/convsamp.cpp
    // see: https://stackoverflow.com/questions/6010793/looking-for-simple-practical-c-examples-of-how-to-use-icu
    icu::UnicodeString ucs = icu::UnicodeString::fromUTF8(icu::StringPiece("abc"));
//    icu_62::UnicodeString ucs = icu_62::UnicodeString::fromUTF8(icu_62::StringPiece("abc"));
//    icu_60::UnicodeString ucs = icu_60::UnicodeString::fromUTF8(icu_60::StringPiece("abc"));
...
//    UConverter* converter = ucnv_open("ISO-8859-1", &status);
    ...
    if (U_SUCCESS(status))
    {
        ...
        cout << "ucnv_getMaxCharSize(): " << static_cast<int>(maxCharSize) << endl;
        int8_t minCharSize = ucnv_getMinCharSize(converter);
        cout << "ucnv_getMinCharSize(): " << static_cast<int>(minCharSize) << endl;

        std:vector<UChar> target{};
        target.resize(vec.size() / minCharSize + 1);
        cout << "target.size(): " << target.size() << endl;
        ucnv_toUChars(converter, target.data(), target.size(), vec.data(), vec.size(), &status);
        cout << "error status: " << u_errorName(status) << " (" << status << ")" << endl;
        // NOLINTNEXTLINE(readability-implicit-bool-conversion)
        if (U_FAILURE(status))
        {
            cout << "ERROR!" << endl;
