...
fn andi_ms_library_read_succeeds() {
    ...
    let admin_data = &reader.read("/0").unwrap();
    assert_eq!("Admin Data", admin_data.name);
    assert_eq!(8, admin_data.parameters.len());
    assert_eq!(
        Parameter::from_str_str("Dataset Completeness", "C1+C2"),
        admin_data.parameters[0]
    );
    assert_eq!(
        Parameter::from_str_str("MS Template Revision", "1.0"),
        admin_data.parameters[1]
    );
    // no administrative_comment
    // no dataset_origin
    ...
    assert_eq!(15, raw_data_global.parameters.len());
    assert_eq!(
        Parameter::from_str_i32("Scan Number", 3),
        raw_data_global.parameters[0]
    );
    // no starting_scan_number
    assert_eq!(
    ...
    assert_eq!(
        Parameter::from_str_str("Mass Axis Data Format", AndiMsDataFormat::Short.to_string()),
        raw_data_global.parameters[11]
    );
    assert_eq!(
        Parameter::from_str_str("Time Axis Data Format", AndiMsDataFormat::Short.to_string()),
        raw_data_global.parameters[12]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Intensity Axis Data Format",
            AndiMsDataFormat::Short.to_string()
        ),
        raw_data_global.parameters[13]
    );
    // no mass_axis_label
    ...
    assert_eq!(None, raw_data_scans.table);
    assert_eq!(
        vec!["0 (m/z: 16-32)", "1 (m/z: 1-18)", "2 (m/z: 1-36)"],
        raw_data_scans.child_node_names
    );

    let raw_data_scan_0 = &reader.read("/5/0").unwrap();
    assert_eq!("0 (m/z: 16-32)", raw_data_scan_0.name);
    assert_eq!(7, raw_data_scan_0.parameters.len());
    assert_eq!(
        Parameter::from_str_str(
            "Resolution Type",
            AndiMsResolutionType::Constant.to_string()
    ...
    // no a_d_sampling_rate
    // no a_d_coaddition_factor
    // no scan_acquisition_time
    // no scan_duration
    assert_eq!(
        Parameter::from_str_f64("Mass Range Min", 16f64),
        raw_data_scan_0.parameters[5]
    );
    assert_eq!(
        Parameter::from_str_f64("Mass Range Max", 32f64),
        raw_data_scan_0.parameters[6]
    );
    // no time_range_min
    // no time_range_max
    ...
    // no resolution
    assert_eq!(
        vec![PointXy::new(16.0f64, 100f64), PointXy::new(32.0f64, 200f64),],
        raw_data_scan_0.data
    );
    assert_eq!(
        vec![
    ...
    // library_data
    let library_data_0 = &reader.read("/5/0/0").unwrap();
    assert_eq!("Library Data", library_data_0.name);
    assert_eq!(19, library_data_0.parameters.len());
    assert_eq!(
        Parameter::from_str_i32("Scan Number", 0),
        library_data_0.parameters[0]
    );
    assert_eq!(
        Parameter::from_str_str("Entry Name", "Entry name 0"),
        library_data_0.parameters[1]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Entry ID",
            String::from_iter(std::iter::repeat(' ').take(31))
        ),
        library_data_0.parameters[2]
    );
    assert_eq!(
        Parameter::from_str_i32("Entry Number", 0),
        library_data_0.parameters[3]
    );
    assert_eq!(
        Parameter::from_str_str(
        ...
        library_data_0.parameters[4]
    );
    assert_eq!(
        Parameter::from_str_str(
            "CAS Name",
            String::from_iter(std::iter::repeat(' ').take(254))
        ),
        library_data_0.parameters[5]
    );
    assert_eq!(
        Parameter::from_str_str("Other Name 0", "Other name 0 0".to_owned()),
        library_data_0.parameters[6]
    );
    assert_eq!(
        Parameter::from_str_str("Other Name 1", "Other name 1 0".to_owned()),
        library_data_0.parameters[7]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Other Name 2",
    ...
    assert_eq!(
        ...
        library_data_0.parameters[9]
    );
    assert_eq!(
        Parameter::from_str_i32("CAS Number", 12345),
        library_data_0.parameters[10]
    );
    assert_eq!(
        Parameter::from_str_str("Chemical Formula", "O2"),
        library_data_0.parameters[11]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Wiswesser Notation",
            String::from_iter(std::iter::repeat(' ').take(127))
        ),
        library_data_0.parameters[12]
    );
    assert_eq!(
        Parameter::from_str_str("SMILES Notation", "O=O"),
        library_data_0.parameters[13]
    );
    assert_eq!(
        Parameter::from_str_str(
    ...
    assert_eq!(
        ...
        library_data_0.parameters[16]
    );
    assert_eq!(
        Parameter::from_str_i32("Nominal Mass", 32),
        library_data_0.parameters[17]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Other Information",
        ...
        library_data_0.parameters[18]
    );
    assert!(library_data_0.data.is_empty());
    assert!(library_data_0.metadata.is_empty());
    assert!(library_data_0.table.is_none());
    assert!(library_data_0.child_node_names.is_empty());

    let library_data_2 = &reader.read("/5/2/0").unwrap();
    assert_eq!("Library Data", library_data_2.name);
    assert_eq!(19, library_data_2.parameters.len());
    assert_eq!(
        Parameter::from_str_i32("Scan Number", 2),
        library_data_2.parameters[0]
    );
    assert_eq!(
        Parameter::from_str_str("Entry Name", "Entry name 2"),
        library_data_2.parameters[1]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Entry ID",
        ...
        library_data_2.parameters[2]
    );
    assert_eq!(
        Parameter::from_str_i32("Entry Number", 2),
        library_data_2.parameters[3]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Source Data File Reference",
    ...
    assert_eq!(
        ...
        library_data_2.parameters[5]
    );
    assert_eq!(
        Parameter::from_str_str("Other Name 0", "Other name 0 2".to_owned()),
        library_data_2.parameters[6]
    );
    assert_eq!(
        Parameter::from_str_str("Other Name 1", "Other name 1 2".to_owned()),
        library_data_2.parameters[7]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Other Name 2",
    ...
    assert_eq!(
        ...
        library_data_2.parameters[9]
    );
    assert_eq!(
        Parameter::from_str_i32("CAS Number", 1234567),
        library_data_2.parameters[10]
    );
    assert_eq!(
        Parameter::from_str_str("Chemical Formula", "HCl"),
        library_data_2.parameters[11]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Wiswesser Notation",
        ...
        library_data_2.parameters[12]
    );
    assert_eq!(
        Parameter::from_str_str("SMILES Notation", "Cl"),
        library_data_2.parameters[13]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Other Structure Notation",
    ...
    assert_eq!(
        ...
        library_data_2.parameters[16]
    );
    assert_eq!(
        Parameter::from_str_i32("Nominal Mass", 36),
        library_data_2.parameters[17]
    );
    assert_eq!(
        Parameter::from_str_str(
            "Other Information",
        ...
        library_data_2.parameters[18]
    );
    assert!(library_data_2.data.is_empty());
    assert!(library_data_2.metadata.is_empty());
