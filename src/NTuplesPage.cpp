...
std::vector<sciformats::jdx::StringLdr> sciformats::jdx::NTuplesPage::getPageVariableLdrs()
{
    return m_pageVariableLdrs;
}

sciformats::jdx::NTuplesPage::VarType sciformats::jdx::NTuplesPage::getDataTableVariableList()
{
    return m_dataTableVariableList;
}

sciformats::jdx::NTuplesPage::Variables sciformats::jdx::NTuplesPage::getDataTableVariables()
{
    return m_dataTableVariables;
}

std::optional<sciformats::jdx::NTuplesPage::PlotDescriptor> sciformats::jdx::NTuplesPage::getDataTablePlotDescriptor()
{
    return m_dataTablePlotDescriptor;
}

std::vector<std::pair<double, double>> sciformats::jdx::NTuplesPage::getDataTable()
{
    if (m_dataTableVariableList == VarType::XppYY
        || m_dataTableVariableList == VarType::XppRR
        || m_dataTableVariableList == VarType::XppII)
    {
        std::function<std::vector<std::pair<double, double>>()> readData = [this]() {
            return readXppYyData();
        };
        // TODO: inline lambda?
...
std::vector<std::pair<double, double>> sciformats::jdx::NTuplesPage::readXppYyData()
{
    ...
    if (yData.size() != nPoints)
    {
        throw ParseException(
            "Mismatch betwee NPOINTS and actual number of points in PAGE. VAR_DIM: "
...
std::vector<std::pair<double, double>> sciformats::jdx::NTuplesPage::readXyXyData()
{
    auto dataTableParams = getDataTableVariables();
    ...
    while (parser.hasNext()) {
        ...
        if (!xyData.empty())
        {
            throw ParseException("Unexpected content in DATA TABLE: " + std::get<std::string>(next));
        }
        // if neither previous condition is true, this is a pure comment at the start of DATA TABLE => continue
    }
    return xyData;
}
...
void sciformats::jdx::NTuplesPage::parse(const std::vector<NTuplesVariables>& nTuplesVars, const std::vector<StringLdr>& blockLdrs, TextReader& reader, std::optional<std::string>& nextLine)
{
    ...
    {
        throw ParseException("Unexpected content found while parsing NTUPLES PAGE: " + nextLine.value_or("<end of file>"));
    }
    ...
    if (label != "DATATABLE")
    {
        throw ParseException("Unexpected content found while parsing NTUPLES PAGE: " + nextLine.value());
    }

    std::tie(m_dataTableVariableList, m_dataTablePlotDescriptor) = parseDataTableVars(value);

    auto findNTuplesVars = [&nTuplesVars] (const std::string& symbol) {
        auto it = std::find_if(std::begin(nTuplesVars), std::end(nTuplesVars),
                    [&symbol] (const NTuplesVariables& vars) { return vars.symbol == symbol; });
        if (it != std::end(nTuplesVars))
        {
            return *it;
        }
        throw ParseException("Could not find NTUPLES parameters for SYMBOL: " + symbol);
    };
    ...
    auto mergedVars = mergeVars(blockLdrs, yNTuplesVars.value(), m_pageVariableLdrs);
...
std::vector<sciformats::jdx::StringLdr> sciformats::jdx::NTuplesPage::parsePageVarLdrs(TextReader& reader, std::optional<std::string>& nextLine)
{
    ...
    {
        auto [label, value] = util::parseLdrStart(nextLine.value());
        if (label == "PAGE" || label == "ENDNTUPLES" || label == "END" || label == "DATATABLE")
        {
            // end of page
            break;
        }
...
std::pair<sciformats::jdx::NTuplesPage::VarType, std::optional<sciformats::jdx::NTuplesPage::PlotDescriptor>> sciformats::jdx::NTuplesPage::parseDataTableVars(const std::string& rawPageVar)
{
    ...
    {
        // empty
        throw ParseException("Missing variable list in DATA TABLE: " + rawPageVar);
    }
    auto segments = util::split(rawPageVarsTrimmed, R"(\)\s*,\s*)", true);
    if (segments.empty() || segments.size() > 2)
    {
        throw ParseException("Unexpected content found at DATA TABLE start: " + rawPageVar);
    }
    ...
    if (segments.size() == 1)
    {
        return std::pair<VarType, std::optional<PlotDescriptor>>{varType, std::optional<PlotDescriptor>{std::nullopt}};
    }
    auto plotDesc = determinePlotDescriptor(segments.at(1));
    return std::pair<VarType, std::optional<PlotDescriptor>>{varType, std::optional<PlotDescriptor>{plotDesc}};
}

sciformats::jdx::NTuplesPage::VarType sciformats::jdx::NTuplesPage::determineVarType(const std::string& varList)
{
    const auto* it = std::find_if(s_varTypeMapping.begin(), s_varTypeMapping.end(),
        [&varList](const auto& mappingItem) { return mappingItem.first == varList;});
    if (it != s_varTypeMapping.end())
    {
        return (*it).second;
    }
    throw ParseException("Unsupported variable type in NTUPLES PAGE: " + varList);
}

sciformats::jdx::NTuplesPage::PlotDescriptor sciformats::jdx::NTuplesPage::determinePlotDescriptor(const std::string& plotDescriptor)
{
    ...
    const auto* it = std::find_if(s_plotDescriptorMapping.begin(), s_plotDescriptorMapping.end(),
        [&noCommentPlotDescriptor](const auto& mappingItem) { return mappingItem.first == noCommentPlotDescriptor;});
    if (it != s_plotDescriptorMapping.end())
    {
        return (*it).second;
    }
    throw ParseException("Illegal plot descriptor in NTUPLES PAGE: "  + noCommentPlotDescriptor);
}

sciformats::jdx::NTuplesVariables sciformats::jdx::NTuplesPage::mergeVars(const std::vector<StringLdr>& blockLdrs, const NTuplesVariables& nTuplesVars, const std::vector<StringLdr>& pageLdrs)
{
    auto ySymbols = {"Y", "R", "I"};
    ...
    else if (std::any_of(ySymbols.begin(), ySymbols.end(), [&nTuplesVars](const std::string& s) {return s == nTuplesVars.symbol;}))
    {
        // use values from block relevant for abscissa
        ...
        // FACTOR <-> YFACTOR        
        // TODO: also check for other symbols but Y? Does not seem relevant for NMR and MS.
        for (const auto& blockLdr : blockLdrs)
        {
    ...
    {
        throw ParseException("Unexpected symbol found during parsing of PAGE: " + nTuplesVars.symbol);
    }
