...
sciformats::jdx::PeakAssignment::PeakAssignment(
    const std::string& stringValue, size_t numVariables)
{
    ...
    if (numVariables == 3)
    {
        ...
        {
            // 3 tokens
            x = token0.value().empty()
                    ? std::numeric_limits<double>::quiet_NaN()
                    : strtod(token0.value().data(), nullptr);
            y = token1.value().empty()
                    ? std::numeric_limits<double>::quiet_NaN()
...
std::optional<std::string> sciformats::jdx::PeakAssignment::parseNextToken(
...
std::string sciformats::jdx::PeakAssignment::parseStringToken(
    const std::string& stringValue, size_t& position)
{
    std::string token{};
...
bool sciformats::jdx::PeakAssignment::isPeakAssignmentStart(
...
bool sciformats::jdx::PeakAssignment::isPeakAssignmentEnd(
    const std::string& stringValue)
{
    std::string value{stringValue};
