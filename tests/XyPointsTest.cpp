...
TEST_CASE("parses unevenly spaced (XY..XY) data", "[XyPoints]")
{
    // "##XYPOINTS= (XY..XY)\r\n"
...
TEST_CASE("parses (XR..XR) data", "[XyPoints]")
{
    // "##XYPOINTS= (XR..XR)\r\n"
    const auto* label = "XYPOINTS";
    const auto* variables = "(XR..XR)";
    std::string input{"450.0, 10.0; 451.0, 11.0\r\n"
                      "##END="};
    auto streamPtr = std::make_unique<std::stringstream>(std::ios_base::in);
    streamPtr->str(input);
    ...
    ldrs.emplace_back("XFACTOR", "2.0");
    ldrs.emplace_back("YFACTOR", "10.0");
    ldrs.emplace_back("NPOINTS", "2");
    auto nextLine = std::optional<std::string>{};
    auto xyPointsRecord
        = sciformats::jdx::XyPoints(label, variables, ldrs, reader, nextLine);

    REQUIRE("(XR..XR)" == xyPointsRecord.getVariableList());
...
TEST_CASE("parses (XI..XI) data", "[XyPoints]")
{
    // "##XYPOINTS= (XI..XI)\r\n"
    const auto* label = "XYPOINTS";
    const auto* variables = "(XI..XI)";
    std::string input{"450.0, 10.0; 451.0, 11.0\r\n"
                      "##END="};
    auto streamPtr = std::make_unique<std::stringstream>(std::ios_base::in);
    streamPtr->str(input);
    ...
    REQUIRE("(XI..XI)" == xyPointsRecord.getVariableList());
